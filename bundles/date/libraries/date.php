...
class Date
{
	...
	public static function get_hours( $max=10 )
	{
		for( $i=0; $i<=$max; $i++ )
		{
			$output[] = $i;
	...
	public static function hour_from_time_diff($date1,$date2)
	{
		$diff = self::diff($date1,$date2);
		return $diff->h;
	}
	
	public static function min_from_time_diff($date1, $date2)
	{
		$diff = self::diff($date1,$date2);
		return $diff->i;
	}
	
	public static function clean_diff($date1, $date2 = null)
	{
		$diff = self::diff($date1,$date2);
